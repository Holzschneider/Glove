...
public class HeapStack implements AutoCloseable {
	...
	protected HeapStack(int capacity) {
		stack = ByteBuffer.allocateDirect(capacity);
	}
...
//	public ShortBuffer shorts(short... values) { return (ShortBuffer) mallocShort(values.length).put(values).flip(); }
	...
	public IntBuffer mallocInt(int num) { return malloc(num<<4).asIntBuffer(); }
	...
	public FloatBuffer mallocFloat(int num) { return malloc(num<<4).asFloatBuffer(); }
	...
	public DoubleBuffer mallocDouble(int num) { return malloc(num<<4).asDoubleBuffer(); }
